...
data<-read.csv("C:\\Users\\Dorcas\\Documents\\CcHUB Education\\relearn\\Raw Data\\long.csv")
...
```
```{r}
      p <- ggplot(average_marks_control, aes(as.factor(term), Performance,fill = Performance)) +
           geom_col() + scale_fill_distiller(palette = "Greens", direction = 1) + 
           ggtitle("Overall Marks over the Year for Treatment Group") + labs(x="Term",y="Average Marks") + 
           theme_minimal() +
           theme(
              panel.grid = element_blank(),
              panel.grid.major.y = element_line(color = "white"),
...
```{r}
      p <- ggplot(average_marks_treatment, aes(as.factor(term), Performance,fill = Performance)) +
           geom_col() + scale_fill_distiller(palette = "Blues", direction = 1) + 
           ggtitle("Overall Marks over the Year for Control Group") + labs(x="Term",y="Average Marks") + 
           theme_minimal() +
           theme(
              panel.grid = element_blank(),
              panel.grid.major.y = element_line(color = "white"),
